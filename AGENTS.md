...
src/chicken拥有chicken_brown作者几何、8骨绑定和idle／walk／run／peck四动作。L2加入三套独立闭合作者LOD：LOD0 140三角形／100逻辑点，LOD1 72／64，LOD2 36／32；每点1权重／1材质，不按大型坐骑提高预算，也不以运行时删面生成低档。src/livestock只负责家畜目录、真实骨骼采样、分LOD共享姿态缓存、群体实例与工作台，不继承MountDefinition，不添加鞍具／骑手／缰绳字段，不改原六种坐骑和人物V5。
