...
src/chicken拥有chicken_brown作者几何、8骨绑定和idle／walk／run／peck四动作。单档140三角形／100逻辑点／420硬边渲染点／每点1权重／1材质，不按大型坐骑提高预算。src/livestock只负责家畜目录、真实骨骼采样、共享姿态缓存、群体实例与工作台，不继承MountDefinition，不添加鞍具／骑手／缰绳字段，不改原六种坐骑和人物V5。

动物工坊下为坐骑本体／骑乘试衣／家畜三栏目，家畜入口?lab=livestock；原两个坐骑装备字段不变。数量1／10／50／100／500、经营俯视、四动作和日常混合用于模型及群体可读性检查。群体是共享静态姿态缓存与实例批次，不是Unity GPU Crowd或正式ECS玩法；没有寻路、碰撞、LOD、逐只剔除或养殖生产逻辑。不能把浏览器帧率写成Unity性能结论。
