...
src/chicken拥有chicken_brown作者几何、8骨绑定和idle／walk／run／peck四动作。当前家禽翼区采用身体侧面大分区：鸡LOD0 140三角形／102逻辑点，LOD1 72／46，LOD2 36／26；鸭LOD0 126／81，LOD1 60／36，LOD2 36／24。LOD0每侧仍4 tris但全部用于朝外零厚度大翼面，低档以主体后/中段侧带顶点色提示；每点1权重／1材质，不按大型坐骑提高预算，也不以运行时删面生成低档。src/livestock只负责家畜目录、真实骨骼采样、分LOD共享姿态缓存、群体实例与工作台，不继承MountDefinition，不添加鞍具／骑手／缰绳字段，不改原六种坐骑和人物V5。
