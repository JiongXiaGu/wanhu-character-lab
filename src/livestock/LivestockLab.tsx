...
import type { CrowdCount, LabOptions, LabStats, LivestockView, Playback } from './types';
...
export default function LivestockLab() {
  ...
  return <main className="horse-lab livestock-lab">
    <header className="horse-topbar"><div><p>WANHU / LIVESTOCK STUDY</p><h1>家畜工坊 <span>Livestock Lab</span></h1></div><WorkspaceSwitcher active="animal"/><span className="horse-phase-tag">L1 · 低模家禽</span></header>
    <div className="horse-workspace">
      <section className="horse-stage" aria-label="家畜模型与群体预览">
        <div className="horse-stage-title"><p>SMALL ANIMAL / LARGE SCENE</p><h2>{definition.name}</h2><span className="horse-stage-motion">{options.count === 1 ? '单只检查' : `${options.count} 只群体预览`} · {mixed ? '日常混合' : active.label}</span></div>
        ...
        <div className="horse-stage-caption"><span>拖动旋转 · 滚轮缩放 · 正交相机</span><span data-testid="livestock-stats">{stats ? `${stats.triangles} tris / 只 · ${stats.logicalVertices} 逻辑点 · ${stats.bones} 骨骼` : '正在生成模型…'}</span></div>
      ...
      <aside className="horse-inspector">
        ...
        <section className="livestock-selector"><p className="horse-eyebrow">01 / LIVESTOCK</p><h2>家畜</h2><label><span>种类</span><select aria-label="家畜种类" value={definition.id} onChange={() => {}}>{LIVESTOCK.map(value => <option key={value.id} value={value.id}>鸡 · {value.name}</option>)}</select></label><p>成年母鸡 · 不可骑乘 · 单档精度</p></section>
        ...
        <section><p className="horse-eyebrow">03 / CROWD</p><h2>群体预览</h2><div className="livestock-counts" aria-label="家畜数量">{CROWD_COUNTS.map(count => <button key={count} data-testid={`livestock-count-${count}`} aria-pressed={options.count === count} onClick={() => chooseCount(count)}>{count}</button>)}</div>
          ...
          <p className="livestock-help">{mixed ? '停驻、行走、啄食错峰分布。点击上方动作可切换为统一检查。' : options.count > 1 ? '统一动作，循环时错开相位；关闭循环后同步检查单次动作。' : '切换数量后自动适配经营俯视，不改变鸡的模型精度。'}</p>
          <div className="livestock-summary" data-testid="livestock-crowd-summary"><div><strong>{options.count}</strong><span>只</span></div><div><strong>{stats?.modelTriangles.toLocaleString() ?? '—'}</strong><span>模型三角形</span></div><div><strong>{stats?.batches ?? '—'}</strong><span>鸡绘制批次</span></div></div>
        </section>
        <details className="livestock-inspection"><summary>模型审查<span>色块 / 素模 / 线框</span></summary><div className="horse-display-options">{([['beauty', '色块'], ['clay', '素模'], ['wire', '线框']] as const).map(([display, label]) => <button key={display} aria-pressed={options.display === display} onClick={() => setOptions(value => ({ ...value, display }))}>{label}</button>)}</div><div className="horse-toggles"><label><input aria-label="家畜骨架" type="checkbox" checked={options.skeleton} disabled={options.count !== 1} onChange={event => setOptions(value => ({ ...value, skeleton: event.target.checked }))}/>骨架</label><label><input aria-label="家畜参考网格" type="checkbox" checked={options.grid} onChange={event => setOptions(value => ({ ...value, grid: event.target.checked }))}/>网格</label></div><p className="livestock-help">骨架只在单只模式显示。实际每点 1 权重，全部共用 1 个鸡材质。</p></details>
        <p className="livestock-boundary">本页验证模型、动作与群体可读性。群体使用共享烘焙姿态，不含寻路、碰撞、养殖玩法或 Unity 性能结论。</p>
      </aside>
    </div>
  </main>;
}
