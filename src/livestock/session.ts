...
import { resolveHabitat } from './habitat';
...
export function readLivestockSession(query: URLSearchParams): { options: LabOptions; camera?: CameraSnapshot } {
  ...
  const definition = LIVESTOCK.find(d => d.id === (query.get('animal') ?? stored.animal)) ?? LIVESTOCK[0];
  const resolved = resolveHabitat(definition, query.get('surface') ?? stored.surface, query.get('clip') ?? stored.motion);
  ...
  const options: LabOptions = {
    count, ...resolved, animal: definition.id, waterline: stored.waterline === true, view, lod, mixed: query.has('mixed') ? query.get('mixed') === '1' : typeof stored.mixed === 'boolean' ? stored.mixed : count > 1,
  ...
  const camera = !query.has('count') && !query.has('view') && !query.has('animal') && !query.has('surface') && raw && validVector(raw.position) && validVector(raw.target) && Number.isFinite(raw.zoom) && raw.zoom >= .2 && raw.zoom <= 15 ? raw : undefined;
