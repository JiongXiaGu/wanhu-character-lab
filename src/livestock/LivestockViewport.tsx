...
export function LivestockViewport({ options, camera, onReport, onError }: { options: LabOptions; camera?: CameraSnapshot; onReport: (stats: LabStats, playback: Playback) => void; onError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    try { const scene = createLivestockScene(host.current, livestockDefinition(current.current.animal), current, (stats, playback) => callbacks.current.onReport(stats, playback), camera); return () => scene.dispose(); }
