...
export default function MountLab() {
  ...
  const [clip, setClip] = useState<MountSelection>(query.get('pose') === 'bind' ? 'bind' : initialMountMotion(query.get('clip')));
  const [playing, setPlaying] = useState(!query.has('paused') && query.get('pose') !== 'bind' && clip !== 'bind');
  const [phase, setPhase] = useState(validPhase(Number(query.get('phase') ?? 0))), [seekRevision, setSeekRevision] = useState(0);
  const [view, setView] = useState<HorseView>(views.some(([id]) => id === query.get('view')) ? query.get('view') as HorseView : 'three'), [viewRevision, setViewRevision] = useState(0);
  const [orthographic, setOrthographic] = useState(true), [speed, setSpeed] = useState(1), [loop, setLoop] = useState(true), [display, setDisplay] = useState<HorseDisplay>('beauty');
  ...
  return <main className="horse-lab mount-lab">
    <header className="horse-topbar"><div><p>WANHU / MOUNT STUDY</p><h1>坐骑工坊 <span>Mount Lab</span></h1></div><WorkspaceSwitcher active="animal"/><span className="horse-phase-tag">PHASE M5 · 多坐骑</span></header>
    <div className="horse-workspace">
      ...
      <aside className="horse-inspector">
        <AnimalModeSwitcher active="horse" mountId={mountId} saddleId={saddleId}/><MountSelector mountId={mountId} onMountChange={setMountId} saddleId={saddleId} onChange={setSaddleId}/>
