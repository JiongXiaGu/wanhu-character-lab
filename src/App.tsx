...
const initialRecipe=()=>{
  const outfit=enumQuery("outfit",PRESET_IDS,"farmer"),equipmentValue=qs.get("equipment");
  let recipe=cleanRecipe({outfit,height:DEFAULT_RECIPE.height,build:DEFAULT_RECIPE.build,palette:DEFAULT_RECIPE.palette,hat:qs.get("hat")==="0"?false:undefined,equipment:equipmentValue==="0"?false:equipmentValue==="1"?true:undefined});
  const keys:(keyof CharacterSlots)[]=["headwear","top","bottom","shoes","back","leftHand","rightHand"],slotPatch:Partial<CharacterSlots>={};let changed=false;
  for(const key of keys){const value=qs.get(key);if(!value)continue;(slotPatch as Record<string,string>)[key]=value;changed=true;}
  if(changed)recipe=cleanRecipe({...recipe,preset:"custom",slots:{...recipe.slots,...slotPatch}});
  return recipe;
};
...
function SlotSelect<K extends keyof CharacterSlots>({slot,label,recipe,onChange}:SlotSelectProps<K>){return <label className="slot-row"><span>{label}</span><select aria-label={label} value={recipe.slots[slot]} onChange={event=>onChange(slot,event.target.value as CharacterSlots[K])}>{SLOT_OPTIONS[slot].map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label>;}
...
export default function App() {
  ...
  const selectPreset=(preset:Outfit)=>setRecipe(current=>applyPreset(current,preset));
  ...
  const selectView=(value:View)=>{setView(value);setViewRevision(current=>current+1);};
  const selectedPreset=recipe.preset==="custom"?null:PRESETS.find(preset=>preset.id===recipe.preset)??null;
  return (
    <main className={"studio "+(activeAction?"work-mode":"")}>
      <header className="topbar"><div className="brand-mark">工</div><div><p className="eyebrow">WANHU / CHARACTER STUDIO</p><h1>万户人物工坊</h1></div><div className="header-right"><span className="status-dot"/>运行时生成 <span className="version">V3.2 · LABOR</span><button className="ghost" onClick={()=>exportRecipe(recipe)}>导出配方 ↗</button></div></header>
      <div className="workspace">
        <aside className="sidebar">
          ...
          <div className="outfit-list">{PRESETS.map(preset=><button key={preset.id} className={"outfit "+(recipe.preset===preset.id?"selected":"")} aria-pressed={recipe.preset===preset.id} onClick={()=>selectPreset(preset.id)}><span className={"outfit-glyph "+preset.id}>{preset.glyph}</span><span><strong>{preset.name}</strong><small>{preset.desc}</small></span><span className="radio"/></button>)}</div>
          ...
          <label className="slider-label">身高 <output>{recipe.height.toFixed(2)} m</output><input aria-label="身高" type="range" min="1.58" max="1.92" step=".01" value={recipe.height} onChange={event=>patch({height:+event.target.value})}/></label>
          <label className="slider-label">体格 <output>{["偏瘦","匀称","壮实"][recipe.build<.33?0:recipe.build>.66?2:1]}</output><input aria-label="体格" type="range" min="0" max="1" step=".05" value={recipe.build} onChange={event=>patch({build:+event.target.value})}/></label>
          ...
          <div className="switch-row"><label htmlFor="skeleton">骨骼叠加</label><input id="skeleton" type="checkbox" checked={skeleton} onChange={event=>setSkeleton(event.target.checked)}/></div>
          <div className="switch-row"><label htmlFor="grid">地面网格</label><input id="grid" type="checkbox" checked={grid} onChange={event=>setGrid(event.target.checked)}/></div>
        ...
        <section className="stage" aria-label="人物预览">
          ...
          <section className="motion-panel">
            <div className="motion-head"><span className="eyebrow">MOTION LAB</span><div className="speed"><label htmlFor="speed">速度</label><select id="speed" value={speed} onChange={event=>setSpeed(+event.target.value)}>{[.25,.5,1,1.5,2].map(value=><option key={value} value={value}>{value}×</option>)}</select></div></div>
            <div className="motion-actions">{(Object.keys(MOTION_LABELS) as Motion[]).map(value=><button className={!activeAction&&motion===value?'active':''} key={value} onClick={()=>{setWorkAction('none');setMotion(value);setPhase(0);}}>{MOTION_LABELS[value]}</button>)}<button className="play-button" aria-label={playing?'暂停':'播放'} onClick={()=>setPlaying(value=>!value)}>{playing?'Ⅱ 暂停':'▶ 播放'}</button></div>
