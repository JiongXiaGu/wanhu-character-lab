...
/** 一个可选资产，内部分别跟随背骨和头骨；不进入人物配方或马Skeleton调色板。 */
export function createSaddleActor(horse: HorseActor, initial: SaddleId = 'none') {
  ...
  bitLeft.position.fromArray(BIT_BIND.left).sub(headBind); bitRight.position.fromArray(BIT_BIND.right).sub(headBind);
  bridleRoot.add(bitLeft, bitRight);
  ...
  const applyDisplay = () => meshes.forEach(mesh => {
    const material = mesh.material as MeshStandardMaterial; material.vertexColors = display !== 'clay'; material.color.set(display === 'clay' ? '#c2b49c' : '#ffffff');
    material.wireframe = display === 'wire'; material.needsUpdate = true;
  });
  const result = {
    root, bridleRoot, bitLeft, bitRight,
    get id() { return id; }, get meshes() { return meshes as readonly Mesh[]; },
    select(next: SaddleId) {
      saddleDefinition(next);
      if (disposed || id === next) return false;
      const candidates: Mesh[] = [];
      try {
        if (next !== 'none') for (const geometry of [buildSaddle(next), buildBridle()]) {
    ...
    stats() { return { id, triangles: meshes.reduce((sum, mesh) => sum + (mesh.geometry.index?.count ?? mesh.geometry.getAttribute('position').count) / 3, 0),
      geometries: meshes.map(mesh => mesh.geometry.uuid) }; },
  ...
  try { result.select(initial); } catch (error) { result.dispose(); throw error; }
  return result;
}
