...
import { isSaddleId, type SaddleId } from './saddles/catalog';
...
export default function HorseLab() {
  ...
  const [saddleId, setSaddleId] = useState<SaddleId>(isSaddleId(saddleQuery) ? saddleQuery : 'none');
  ...
  return <main className="horse-lab">
    <header className="horse-topbar"><div><p>WANHU / ANIMAL STUDY</p><h1>马匹实验 <span>Horse Lab</span></h1></div><WorkspaceSwitcher active="animal"/><span className="horse-phase-tag">PHASE M3 · 马匹与马鞍</span></header>
    <div className="horse-workspace">
      <section className="horse-stage" aria-label="马匹预览">
        ...
        <nav className="horse-cameras" data-testid="horse-cameras" aria-label="马匹相机">{views.map(([id, label]) => <button key={id} data-testid={'horse-view-' + id} aria-pressed={view === id} onClick={() => { setView(id); setViewRevision(n => n + 1); }}>{label}</button>)}<button data-testid="horse-projection" onClick={() => setOrthographic(value => !value)}>{orthographic ? '正交' : '透视'}</button></nav>
        {error ? <div role="alert" className="horse-error">{error}<button onClick={() => location.reload()}>重新载入</button></div> : <HorseViewport options={{ saddleId, clip, playing, speed, phase, seekRevision, loop, view, viewRevision, orthographic, display, skeleton, grid }} onStats={setStats} onPlayback={report} onError={setError}/>}
        ...
        <section className="horse-transport" aria-label="马动画播放控制"><div className="horse-transport-row">
          <button data-testid="horse-play" disabled={!active} onClick={() => playback.finished ? replay() : setPlaying(value => !value)}>{playing ? 'Ⅱ 暂停' : '▶ 播放'}</button><button data-testid="horse-replay" disabled={!active} onClick={replay}>重播</button>
          <button data-testid="horse-previous" disabled={!active} onClick={() => seek(playback.phase - 1 / (HORSE_FPS * (playback.duration || 1)))}>上一帧</button><button data-testid="horse-next" disabled={!active} onClick={() => seek(playback.phase + 1 / (HORSE_FPS * (playback.duration || 1)))}>下一帧</button>
          <label className="horse-loop"><input type="checkbox" checked={loop} onChange={event => setLoop(event.target.checked)} aria-label="马动画循环播放"/>循环播放</label><label className="horse-speed">速度<select aria-label="马动画速度" value={speed} onChange={event => setSpeed(Number(event.target.value))}>{[.25, .5, 1, 1.5, 2].map(value => <option key={value} value={value}>{value}×</option>)}</select></label>
        </div><label className="horse-timeline"><span>相位</span><input aria-label="马动画相位" type="range" min="0" max="1" step=".001" disabled={!active} value={playback.phase} onChange={event => seek(Number(event.target.value))}/><output>{(playback.phase * 100).toFixed(1)}%</output></label><div className="horse-time" data-testid="horse-time">{playback.time.toFixed(2)} / {playback.duration.toFixed(2)} 秒 <span>{active ? playback.finished ? '结束保持' : loop ? '原地循环 · 无场景位移' : '单次播放 · 结束保持' : '绑定姿态 · 不播放动画'}</span></div></section>
      </section>
      <aside className="horse-inspector">
        <AnimalModeSwitcher active="horse" saddleId={saddleId}/><MountSelector saddleId={saddleId} onChange={setSaddleId}/>
        <section><p className="horse-eyebrow">02 / MOTION</p><h2>基础动作</h2><div className="horse-clip-list">{HORSE_CLIPS.map(definition => <button key={definition.id} data-testid={definition.id} aria-pressed={clip === definition.id} onClick={() => selectClip(definition.id)}><strong>{definition.label}</strong><small>{definition.description}</small><span>{definition.duration.toFixed(1)} s</span></button>)}</div><button className="horse-bind-button" data-testid="horse-bind" aria-pressed={clip === 'bind'} onClick={() => selectClip('bind')}>查看静态绑定姿态</button></section>
        <section><p className="horse-eyebrow">03 / INSPECTION</p><h2>模型审查</h2><div className="horse-display-options">{([['beauty', '色块'], ['clay', '素模'], ['wire', '线框']] as [HorseDisplay, string][]).map(([id, label]) => <button key={id} data-testid={'horse-display-' + id} aria-pressed={display === id} onClick={() => setDisplay(id)}>{label}</button>)}</div><div className="horse-toggles"><label><input type="checkbox" checked={skeleton} onChange={event => setSkeleton(event.target.checked)} aria-label="马骨架"/>骨架</label><label><input type="checkbox" checked={grid} onChange={event => setGrid(event.target.checked)} aria-label="马参考网格"/>参考网格</label></div><dl><div><dt>逻辑顶点</dt><dd>{stats?.logicalVertices ?? '—'}</dd></div><div><dt>三角形</dt><dd>{stats?.triangles ?? '—'}</dd></div><div><dt>骨骼 / 每点权重</dt><dd>{stats?.bones ?? '—'} / ≤ 2</dd></div><div><dt>单位 / 朝向</dt><dd>米 / +Z 前</dd></div></dl></section>
        <section className="horse-boundary"><p className="horse-eyebrow">马匹本体范围</p><p>这里独立检查马、马鞍与辔头。没有骑手时不显示悬空缰绳；切换“骑乘试衣”查看掌心到马嘴的动态连接。</p><p>四个马动作保持，无马鞍也能播放；这是原地实验，不是完整移动系统。</p></section>
      </aside>
    </div>
  </main>;
}
