...
export interface HorseViewOptions {
  clip: HorseClipId | 'bind'; playing: boolean; speed: number; phase: number; seekRevision: number;
  loop: boolean; view: HorseView; viewRevision: number; orthographic: boolean;
  display: HorseDisplay; skeleton: boolean; grid: boolean;
}
...
interface Runtime {
  actor: HorseActor; player: HorsePlayer; renderer: T.WebGLRenderer; scene: T.Scene;
...
export interface HorseReview {
  ...
  cameraState: () => unknown; jointPositions: () => Record<string, number[]>; matricesFinite: () => boolean;
}
declare global { interface Window { __HORSE_REVIEW__?: HorseReview } }
const DIRECTIONS: Record<HorseView, [number, number, number]> = {
  front: [0, .18, 6], left: [-6, .18, 0], right: [6, .18, 0], back: [0, .18, -6],
  three: [4.6, 2.1, 5.2], 'rear-three': [-4.6, 2.1, -5.2],
};
function applyCamera(runtime: Runtime, options: HorseViewOptions) {
  ...
  if (next !== runtime.camera) {
    runtime.controls.dispose(); runtime.camera = next;
    runtime.controls = new OrbitControls(next, runtime.renderer.domElement);
    runtime.controls.enableDamping = true; runtime.controls.minDistance = 2.6; runtime.controls.maxDistance = 12;
    runtime.controls.minZoom = .5; runtime.controls.maxZoom = 4;
  }
  ...
  runtime.camera.lookAt(runtime.controls.target); runtime.camera.updateProjectionMatrix(); runtime.controls.update();
  runtime.resize(); runtime.render();
}
/** 马的画幅与骨架独立；人物Viewport仍拥有FBX对照等专有职责，不硬套其20骨架接口。 */
export function HorseViewport({ options, onStats, onPlayback, onError }: Props) {
  ...
  useEffect(() => {
    ...
    let actor: HorseActor;
    try { actor = createHorseActor(); }
    catch (reason) {
      renderer.dispose(); renderer.domElement.remove(); floor.geometry.dispose(); floor.material.dispose();
      grid.geometry.dispose(); (grid.material as T.Material).dispose(); error.current(String(reason)); return;
    }
    ...
    const rt: Runtime = { actor, player, renderer, scene, perspective, ortho, camera: ortho, controls, grid,
      resize() {
        ...
        const half = Math.max(1.48, 1.72 / aspect);
    ...
    if (import.meta.env.DEV || new URLSearchParams(location.search).has('review')) {
      window.__HORSE_REVIEW__ = {
        stats: actor.stats,
        seek(phase) { player.seek(phase); rt.render(); report.current(player.status()); },
        getStatus: () => player.status(), geometryId: () => actor.mesh.geometry.uuid,
    ...
    return () => {
      stopped = true; cancelAnimationFrame(frame); observer.disconnect(); rt.controls.dispose(); player.dispose(); actor.dispose();
      ...
      runtime.current = null; delete window.__HORSE_REVIEW__;
    };
  }, []);
  useEffect(() => {
    const rt = runtime.current; if (!rt) return;
    rt.player.select(options.clip); rt.player.setLoop(options.loop); rt.player.seek(options.phase);
  ...
  useEffect(() => {
    ...
    rt.actor.helper.visible = options.skeleton; rt.grid.visible = options.grid; rt.render();
