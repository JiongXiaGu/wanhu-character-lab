...
function hat(c: Cage, outfit: Recipe["outfit"]) {
  ...
  if (outfit === "farmer") {
    ...
    const peak = vertex(c, "StrawPeak", [0, 1.89, 0], skin),
      bottom = vertex(c, "StrawInside", [0, 1.816, 0], skin);
    for (let i = 0; i < 12; i++) {
      const j = (i + 1) % 12;
      face(
        c,
...
function equipment(c: Cage, outfit: Recipe["outfit"]) {
  if (outfit === "guard") {
    ...
    const sw = rigid(B.LeftHand),
      center: Vec3 = [-0.56, 0.87, 0.088];
    const edge = OCT.map(([xx, yy], i) =>
      vertex(c, `Shield.${i}`, add(center, [xx * 0.2, yy * 0.25, 0]), sw),
    );
    const boss = vertex(c, "ShieldBoss", add(center, [0, 0, 0.058]), sw);
    for (let i = 0; i < 8; i++)
      face(
        c,
    ...
    tube(c, "Bow", pts, 0.014, w, "#956d42");
    tube(c, "Bowstring", [pts[0], pts.at(-1)!], 0.002, w, "#d6c9ad", 3);
...
export function makeCharacter(input: Partial<Recipe>): CharacterData {
  const recipe = cleanRecipe(input),
    body = makeBody(),
    c = cloneCage(body);
  const [cloth, trim, leather] = palettes[recipe.palette];
  const dressed = recipe.outfit !== "body",
    armor = recipe.outfit === "guard";
  for (const f of c.faces) {
    f.color = SKIN;
    if (dressed)
      f.color = ["torso", "upperArm"].includes(f.region)
        ? armor
          ? "#566561"
          : recipe.outfit === "archer"
            ? "#756247"
            : cloth
        : ["pelvis", "thigh"].includes(f.region)
          ? "#3c4445"
          : f.region === "shin"
            ? "#8d8b77"
            : f.region === "foot"
  ...
  if (dressed) {
    for (const [name, loop] of Object.entries(c.anchors)) {
      let factor = 1;
      if (["Hip", "Waist", "Rib", "Chest", "Shoulder"].includes(name))
        factor = 1.055;
      ...
      for (const id of loop) {
        const v = c.vertices[id];
        v.p = add(center, mul(sub(v.p, center), factor));
      }
    }
    belt(c, leather);
    ribbon(
  ...
  const replacedTriangles = dressed
    ? body.faces
        .filter((f) =>
          ["torso", "upperArm", "pelvis", "thigh", "shin", "foot"].includes(
            f.region,
          ),
        )
