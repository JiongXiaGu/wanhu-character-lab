import { B, rigid, type Cage, type Joint, type Recipe, type Vec3, type Weight } from './types';
import { OCT, HEX, LEG, add, mul, ring, bridge, face, vertex, orient } from './cage';
...
export function makeBody():Cage {
  ...
  const rows: [string,number,number,number,Weight][] = [
    ['Hip',.91,.165,.104,rigid(B.Hips)], ['Waist',1.055,.142,.083,[B.Hips,B.Spine,.35]],
    ['Rib',1.18,.177,.105,[B.Spine,B.Chest,.35]], ['Chest',1.30,.202,.113,rigid(B.Chest)],
    ['Shoulder',1.395,.211,.098,rigid(B.Chest)], ['NeckBase',1.455,.060,.053,[B.Chest,B.Neck,.35]],
    ['NeckTop',1.51,.057,.055,[B.Neck,B.Head,.3]], ['Jaw',1.545,.076,.079,rigid(B.Head)],
    ['Cheek',1.63,.098,.092,rigid(B.Head)], ['Forehead',1.72,.097,.09,rigid(B.Head)],
  ...
  for(let r=0;r<loops.length-1;r++) {
    for(let j=0;j<8;j++) {
      ...
      face(c,[loops[r][j],loops[r][(j+1)%8],loops[r+1][(j+1)%8],loops[r+1][j]],r<1?'pelvis':r<5?'torso':r<7?'neck':'head');
    }
  }
  face(c,[...loops.at(-1)!],'head');
  const hips=loops[0];
  const crotch=vertex(c,'Crotch',[0,.815,0],rigid(B.Hips));
  for(const side of [1,-1]) {
    ...
    let prev=socket;
    armRows.forEach(([id,center,w,d,skin],i)=>{const next=ring(c,`${prefix}${id}`,center,u,[0,0,1],HEX,w,d,skin);bridge(c,prev,next,i<2?'upperArm':i<5?'forearm':'hand');c.anchors[`${prefix}${id}`]=next;prev=next;});
    face(c,[...prev],'hand');
    const thigh=right?B.RightThigh:B.LeftThigh, shin=right?B.RightShin:B.LeftShin, foot=right?B.RightFoot:B.LeftFoot;
    const legRoot=right?[hips[0],hips[1],hips[2],hips[3],hips[4],crotch]:[hips[4],hips[5],hips[6],hips[7],hips[0],crotch];
    ...
    const legRows: [string,number,number,number,number,Weight][] = [
      ...
      ['KneeUpper',.529,.058,.057,0,[thigh,shin,.94]],
      ['Knee',.489,.055,.055,0,[thigh,shin,.5]],
      ['KneeLower',.449,.056,.052,0,[thigh,shin,.06]],
      ['Calf',.293,.062,.063,-.008,rigid(shin)],
      ['Ankle',.105,.040,.039,0,[shin,foot,.2]],
      ['Instep',.064,.052,.104,.05,rigid(foot)],
      ['Sole',.018,.054,.109,.053,rigid(foot)],
    ];
    prev=legRoot;
    legRows.forEach(([id,y,w,d,z,skin],i)=>{const prof=right?LEG:LEG.map(([x,zz])=>[-x,-zz] as [number,number]);const next=ring(c,`${prefix}${id}`,[side*.101,y,z],[1,0,0],[0,0,1],prof,w,d,skin);bridge(c,prev,next,i<2?'thigh':i<6?'shin':'foot');c.anchors[`${prefix}${id}`]=next;prev=next;});
    face(c,[...prev],'foot');
  }
  orient(c);
  return c;
...
export function shapePoint(p:Vec3,recipe:Recipe):Vec3 {
  const h=recipe.height/1.76, bulk=.88+recipe.build*.24;
  return [p[0]*h*(p[1]>1.51?1:bulk),p[1]*h,p[2]*h*(p[1]>1.51?1:bulk)];
}
export function makeJoints(recipe:Recipe):Joint[] {
  ...
  for(const s of [1,-1]) {const name=s===1?'Right':'Left', clav=j.length;put(`${name}Clavicle`,3,[s*.08,1.36,0]);put(`${name}UpperArm`,clav,[s*.211,1.365,0]);put(`${name}Forearm`,clav+1,[s*.394,1.101,0]);put(`${name}Hand`,clav+2,[s*.503,.912,.014]);}
  for(const s of [1,-1]) {const name=s===1?'Right':'Left',t=j.length;put(`${name}Thigh`,1,[s*.101,.929,0]);put(`${name}Shin`,t,[s*.101,.489,0]);put(`${name}Foot`,t+1,[s*.101,.105,0]);}
  return j;
}
