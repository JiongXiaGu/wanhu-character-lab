...
export function makeFootwear(recipe:Recipe):GarmentPiece|undefined {
  ...
  if(id==='body')return;
  const c:Cage={vertices:[],faces:[],anchors:{}},openings:Record<string,number[]>={},sealedInterfaces:Record<string,number[]>={};
  for(const side of [1,-1]){
    ...
    bridge(c,top,mid,'foot',color);bridge(c,mid,sole,'foot',color);face(c,sole,'foot',color);
    if(id==='cloth_shoes'){face(c,[...top],'foot',color);sealedInterfaces[name+'Ankle']=top;}
    else openings[name+'Ankle']=top;
  }
  orient(c);c.anchors={...openings,...sealedInterfaces};
  return {id,slot:'shoes',version:GARMENT_GEOMETRY_VERSION,mesh:c,covers:['foot'],openings,...(id==='cloth_shoes'?{sealedInterfaces}:{})};
}
