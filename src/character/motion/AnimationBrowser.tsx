...
import {MOTION_CLIPS,type MotionSelection,type MotionSource} from './catalog';
import './animation-browser.css';
const KEY='wanhu.motion-favorites.v3';
const sourceLabel=(source:MotionSource)=>source==='system-animator-glb'?'GLB':'FBX';
export function AnimationBrowser({value,onSelect}:{value:MotionSelection;onSelect:(v:MotionSelection)=>void}){
 ...
 const index=filtered.findIndex(c=>c.id===value),firstGlb=MOTION_CLIPS.find(c=>c.source==='system-animator-glb');
 const toggle=(id:string)=>setFavorites(a=>{const next=a.includes(id)?a.filter(v=>v!==id):[...a,id];try{localStorage.setItem(KEY,JSON.stringify(next));}catch{}return next;});
 ...
 return <section className="animation-library" aria-label="人物动作库">
  ...
  <input type="search" aria-label="搜索动画" placeholder="名称、分类、FBX 或 GLB 文件名" value={search} onChange={e=>setSearch(e.target.value)}/>
  <div className="animation-source-tabs" aria-label="动画来源"><button aria-pressed={source==='all'} onClick={()=>selectSource('all')}>全部</button><button aria-pressed={source==='mixamo-fbx'} onClick={()=>selectSource('mixamo-fbx')}>FBX</button><button aria-pressed={source==='system-animator-glb'} onClick={()=>selectSource('system-animator-glb')}>GLB</button></div>
  ...
  <div className="animation-quick">{[['pilot-switches','坐姿'],['shooting-arrow','射箭'],['jogging','慢跑']].filter(([id])=>MOTION_CLIPS.some(c=>c.id===id)).map(([id,label])=><button key={id} onClick={()=>onSelect(id)}>{label}</button>)}{firstGlb&&<button data-testid="quick-glb" onClick={()=>onSelect(firstGlb.id)}>GLB 验证</button>}</div>
  ...
  <p className="hint">Mixamo FBX 与 XR Animator GLB 共用同一套 20 骨骼播放器；GLB 先按真实 Bind Pose 离线标准化。</p>
 </section>;
}
