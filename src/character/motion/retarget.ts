...
export function retargetMotion(data:CharacterData,source:HumanoidMotionData):RetargetBake{
 validateMotionData(source,source.id);const joints=data.joints,def=motionDefinition(source.id),count=source.times.length,isSystemAnimator=source.source.provider==='SystemAnimator',correct=calibration(joints,source),scale=joints[1].p[1]/source.bindPositions[4];if(isSystemAnimator)correct[1].identity();
 ...
 if(def.ground&&source.source.provider==='SystemAnimator'){const sorted=[...minYs].sort((a,b)=>a-b),sample=sorted[Math.min(sorted.length-1,Math.floor(sorted.length*.1))],fixed=Math.max(0,-sample);for(let f=0;f<count;f++){groundLift.push(fixed);hips[f*3+1]+=fixed;}if(fixed)bounds.translate(new T.Vector3(0,fixed,0));bounds.expandByScalar(.1);}
