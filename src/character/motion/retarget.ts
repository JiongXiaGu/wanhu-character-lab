...
export interface RetargetBake{
  clip:T.AnimationClip;rotations:number[][];hips:number[];sourcePositions:Float32Array;scale:number;
  groundLift:number[];rootTrajectory:number[];seamDegrees:number;loop:boolean;bounds:T.Box3;
}
const v=(array:readonly number[],offset:number)=>new T.Vector3(array[offset],array[offset+1],array[offset+2]);

/** Mixamo 的 A/T Pose 差异继续使用既有方向校准；XR Animator GLB 使用真实 Bind World Delta，不做此校准。 */
export function calibration(joints:Joint[],source:HumanoidMotionData):T.Quaternion[]{
  if(joints.length!==20)throw new Error('人物动作重定向要求固定 20 骨骼。');
  if(source.source.profile!=='mixamo-fbx-v2')return joints.map(()=>new T.Quaternion());
...
export function retargetMotion(data:CharacterData,source:HumanoidMotionData):RetargetBake{
  validateMotionData(source,source.id);
  const joints=data.joints,def=motionDefinition(source.id),count=source.times.length,isMixamo=source.source.profile==='mixamo-fbx-v2';
  ...
  const rotations=joints.map(()=>[] as number[]),hips:number[]=[],groundLift:number[]=[],rootTrajectory:number[]=[],rawMinY:number[]=[];
  ...
  for(let f=0;f<count;f++){
    const phase=source.times[f]/source.duration;trend.lerpVectors(sourceStart,sourceEnd,phase);
    ...
    root.add(localBind[1]);rootTrajectory.push((trend.x-sourceStart.x)*scale,0,(trend.z-sourceStart.z)*scale);
    for(let i=0;i<20;i++){
      globalQ[i].fromArray(source.worldDeltas,(f*20+i)*4);
      if(isMixamo)globalQ[i].multiply(correct[i]);
      globalQ[i].normalize();
      if(isMixamo&&i===2){
        const expected=v(source.positions,(f*SAMPLE_BONE_COUNT+3)*3).sub(v(source.positions,(f*SAMPLE_BONE_COUNT+2)*3)).normalize();
        swing.setFromUnitVectors(spineDirection.clone().applyQuaternion(globalQ[i]),expected);globalQ[i].premultiply(swing).normalize();
      }
      const parent=joints[i].parent;
      q.copy(parent<0?globalQ[i]:globalQ[parent]).invert().multiply(globalQ[i]);
      if(parent<0)q.copy(globalQ[i]);q.normalize();
    ...
    rawMinY.push(minY);
    const lift=isMixamo&&def.ground?Math.max(0,-minY):0;
  ...
  if(!isMixamo&&def.ground){
    const sorted=[...rawMinY].sort((a,b)=>a-b),baseline=Math.max(0,-sorted[Math.min(sorted.length-1,Math.floor(sorted.length*.1))]);
    for(let f=0;f<count;f++){hips[f*3+1]+=baseline;groundLift[f]=baseline;}
    bounds.translate(new T.Vector3(0,baseline,0));
  }
  ...
  let seamDegrees=0;for(const values of rotations)seamDegrees=Math.max(seamDegrees,new T.Quaternion().fromArray(values).angleTo(new T.Quaternion().fromArray(values,values.length-4))*180/Math.PI);
...
export function exportTargetMotion(data:CharacterData,source:HumanoidMotionData,bake:RetargetBake){
  return{
    schema:'wanhu-target-motion',version:2,retargetVersion:RETARGET_VERSION,skeletonVersion:'wanhu-20-v1',
    calibrationProfile:{id:data.recipe.bodyType==='female'?'female-anatomical-v1':'male-anatomical-v2',source:source.source.profile,head:'source-world-bind-delta'},
    ...
    bones:data.joints.map((joint,i)=>({id:i,name:joint.name,parent:joint.parent,bindLocalPosition:v(joint.p,0).sub(joint.parent<0?new T.Vector3():v(data.joints[joint.parent].p,0)).toArray(),bindLocalRotation:[0,0,0,1],rotations:bake.rotations[i],...(i===1?{positions:bake.hips}:{})})),
    rootTrajectory:bake.rootTrajectory,events:[],props:[],
    limitations:['No fingers/toes','No authored prop or gameplay event tracks','Not a Unity runtime package','No foot lock in G1'],
  };
}
