...
import type {Recipe,CharacterSlots} from '../v3/types';
...
export function createWorkPlayer(actor:Actor,id:WorkId,recipe:Recipe):WorkPlayer {
  ...
  const sync=()=>{
    ...
    if(props.wheel)props.wheel.rotation.x=(id==='pull'?-1:1)*elapsed*(.54/1.3)/(.225*(recipe.height/1.76));
