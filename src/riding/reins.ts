...
import { horseBone } from '../horse/rig';
...
/** 两根闭合细带，固定BufferGeometry；只更新已有顶点，不模拟绳索也不执行手部IK。 */
export function createReins(horse: HorseActor, tack: SaddleActor, initialRider: Actor) {
  const grips = [new Group(), new Group()], guide = new Group();
  grips[0].name = 'LeftReinGrip'; grips[1].name = 'RightReinGrip'; guide.name = 'ReinNeckGuide';
  horse.bones[horseBone('NeckUpper')].add(guide);
  ...
  for (let side = 0; side < 2; side++) {
    ...
    for (let i = 0; i < REIN_SEGMENTS; i++) for (let j = 0; j < 4; j++) {
      const a = base + i * 4 + j, b = base + (i + 1) * 4 + j, c = base + (i + 1) * 4 + (j + 1) % 4, d = base + i * 4 + (j + 1) % 4;
      indices.push(a, c, b, a, d, c);
    }
    indices.push(base, base + 2, base + 1, base, base + 3, base + 2);
    const end = base + REIN_SEGMENTS * 4; indices.push(end, end + 1, end + 2, end, end + 2, end + 3);
  }
  const geometry = new BufferGeometry();
  geometry.setAttribute('position', new BufferAttribute(positions, 3).setUsage(DynamicDrawUsage));
  geometry.setAttribute('normal', new BufferAttribute(normals, 3).setUsage(DynamicDrawUsage)); geometry.setIndex(indices);
  const material = new MeshStandardMaterial({ color: '#493729', roughness: .96 });
  const mesh = new Mesh(geometry, material); mesh.name = 'Reins'; mesh.castShadow = true; mesh.frustumCulled = false; horse.mesh.add(mesh);
  ...
  const bindRider = (rider: Actor) => {
    for (let i = 0; i < 2; i++) {
      const sign = i === 0 ? -1 : 1, index = i === 0 ? B.LeftHand : B.RightHand;
      grips[i].position.fromArray(shapePoint([sign * .528, .864, .020], rider.data.recipe)).sub(new Vector3(...rider.data.joints[index].p));
      rider.bones[index].add(grips[i]);
    }
  };
  bindRider(initialRider);
  return {
    mesh, grips, guide, points,
    setRider: bindRider,
    ...
    update(selection: RidingSelection) {
      ...
      // 马和骑手已在同帧采样。运算落回整马局部空间，支持实例平移、旋转和等比缩放。
      inverse.copy(horse.mesh.matrixWorld).invert();
      const sag = selection === 'Rider_Run' ? .025 : selection === 'Rider_Walk' ? .043 : .060;
      for (let side = 0; side < 2; side++) {
        ...
        middle.set(sign * .275, .085, .03).applyMatrix4(guide.matrixWorld).applyMatrix4(inverse);
        // 三个语义点派生两控制点，绕颈侧而非用两端直线切穿马颈。
        c1.lerpVectors(start, middle, .75); c2.lerpVectors(end, middle, .75);
        c1.x = sign * Math.max(sign * c1.x, sign * middle.x + .065); c2.x = sign * Math.max(sign * c2.x, sign * middle.x + .065);
        for (let i = 0; i <= REIN_SEGMENTS; i++) {
          const t = i / REIN_SEGMENTS, u = 1 - t, point = points[side][i];
          point.copy(start).multiplyScalar(u * u * u).addScaledVector(c1, 3 * u * u * t).addScaledVector(c2, 3 * u * t * t).addScaledVector(end, t * t * t);
          point.y -= sag * Math.sin(Math.PI * t) ** 2;
        }
        for (let i = 0; i <= REIN_SEGMENTS; i++) {
          ...
          for (let j = 0; j < 4; j++) {
            const angle = j * Math.PI / 2, index = ((side * (REIN_SEGMENTS + 1) + i) * 4 + j) * 3;
            radial.copy(normal).multiplyScalar(Math.cos(angle)).addScaledVector(binormal, Math.sin(angle));
            normals[index] = radial.x; normals[index + 1] = radial.y; normals[index + 2] = radial.z;
            positions[index] = points[side][i].x + radial.x * REIN_RADIUS; positions[index + 1] = points[side][i].y + radial.y * REIN_RADIUS; positions[index + 2] = points[side][i].z + radial.z * REIN_RADIUS;
          }
        }
      }
