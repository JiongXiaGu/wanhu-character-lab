...
interface Runtime {
  player: RidingPlayer; scene: T.Scene; renderer: T.WebGLRenderer; controls: OrbitControls;
  camera: T.PerspectiveCamera | T.OrthographicCamera; perspective: T.PerspectiveCamera; ortho: T.OrthographicCamera;
  grid: T.GridHelper; seatHelper: T.AxesHelper; resize: () => void; render: () => void;
}
...
const DIRECTIONS: Record<HorseView, [number, number, number]> = {
  front: [0, .15, 7.6], left: [-7.6, .15, 0], right: [7.6, .15, 0], back: [0, .15, -7.6],
  three: [5.3, 2.3, 6.5], 'rear-three': [-5.3, 2.3, -6.5],
};
function applyCamera(rt: Runtime, options: RidingViewOptions) {
  ...
  rt.camera.position.copy(rt.controls.target).add(new T.Vector3(...DIRECTIONS[options.view]));
...
function applyDisplay(rt: Runtime, options: RidingViewOptions) {
  for (const material of [rt.player.horse.material, rt.player.rider.beautyMaterial]) {
...
export function RidingViewport({ options, onStats, onPlayback, onError }: Props) {
  ...
  useEffect(() => {
    ...
    catch (reason) { error.current(`无法创建骑乘WebGL预览：${String(reason)}`); return; }
    let player: RidingPlayer;
    try { player = createRidingPlayer(latest.current.recipe); }
    catch (reason) { renderer.dispose(); error.current(String(reason)); return; }
    ...
    const key = new T.DirectionalLight('#fff0d8', 2.8); key.position.set(-3.5, 6, 5); key.castShadow = true;
    key.shadow.mapSize.set(1024, 1024); Object.assign(key.shadow.camera, { left: -3, right: 3, top: 3.5, bottom: -3 }); key.shadow.bias = -.0005;
