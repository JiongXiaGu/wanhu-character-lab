...
const definition = LIVESTOCK[0], actor = createAnimalActor(definition), data = actor.data;
assert.equal(data.indices.length / 3, 140); assert.equal(data.positions.length, 100); assert.equal(actor.bones.length, 8);
assert.equal(actor.geometry.getAttribute('position').count, 420); assert.equal(actor.geometry.groups.length, 0);
...
const edgeCounts = new Map<string, number>(), signedEdges = new Map<string, number>();
for (let i = 0; i < data.indices.length; i += 3) {
  const triangle = data.indices.slice(i, i + 3), [a, b, c] = triangle.map(index => new Vector3(...data.positions[index]));
  assert(b.clone().sub(a).cross(c.clone().sub(a)).lengthSq() > 1e-12, '退化三角形');
  for (let j = 0; j < 3; j++) {
...
assert([...edgeCounts.values()].every(value => value === 2), '所有小壳必须闭合');
assert([...signedEdges.values()].every(value => value === 0), '相邻面绕序必须一致');
const pos = actor.geometry.getAttribute('position'), point = new Vector3();
function sample(motion: string, phase: number) {
...
actor.bind();
for (let i = 0; i < pos.count; i++) { point.fromBufferAttribute(pos, i); const original = point.clone(); actor.mesh.applyBoneTransform(i, point); assert(point.distanceTo(original) < 1e-6, 'inverse bind失真'); }
assert.throws(() => actor.sample('unknown', 0));
const cache = createPoseCache(definition), size = cache.size;
for (const motion of definition.motions) {
