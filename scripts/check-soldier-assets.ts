...
import {B,BODY_TYPES,HAIR_STYLE_IDS,createRecipe,type Cage,type Recipe} from '../src/character/v3/types';
...
function closedRigid(c:Cage,bone:number,count:number){
  ...
  for(const v of c.vertices){assert(v.p.every(Number.isFinite));assert.deepEqual(v.w,[bone,bone,1]);}
...
for(const bodyType of BODY_TYPES)for(const hairStyle of HAIR_STYLE_IDS){
  ...
  for(const side of ['Right','Left'])for(const panel of ['Front','Outer','Back'])closedRigid(subset(d.surface,`PalaceTasset.${side}.${panel}.`),side==='Right'?B.RightThigh:B.LeftThigh,20);
