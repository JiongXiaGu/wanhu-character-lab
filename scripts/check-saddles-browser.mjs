...
try {
  ...
  assert.equal(await page.locator('.mount-selector select').count(), 2); assert.deepEqual(await page.getByTestId('mount-horse').locator('option').evaluateAll(nodes => nodes.map(n => n.value)), ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow']);
