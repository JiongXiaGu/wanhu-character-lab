...
import { checkYak } from './mount-yak-checks';
...
try {
  assert.deepEqual(MOUNTS.map(d => d.id), ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'yak_black', 'buffalo_water']); assert.throws(() => mountDefinition('camel' as MountId)); faults++;
