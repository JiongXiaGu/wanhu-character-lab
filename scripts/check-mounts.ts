...
try {
  assert.deepEqual(MOUNTS.map(d => d.id), ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'yak_black']); assert.throws(() => mountDefinition('camel' as MountId)); faults++;
