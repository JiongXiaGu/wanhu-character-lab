...
import { YAK_JOINTS, yakBone } from '../src/yak/rig';
...
import { YAK_AUTHOR_STATS, checkYakAnatomy, checkYakFurAttachments } from './mount-yak-anatomy-checks';
...
function facialContact(actor: MountActor, part: string): number {
  ...
  for (const i of ids) {
    const vertex = actor.data.vertices[i]; assert.deepEqual(vertex.weight, [yakBone('Head'), yakBone('Head'), 1]); center.add(new Vector3(...vertex.position));
  }
...
function checkRider(player: RidingPlayer, intersections: boolean) {
  ...
  assert(player.seat.parent === player.mount.bones[yakBone('Spine')]);
  ...
  for (const [k, f, sign] of [[B.RightShin, B.RightFoot, 1], [B.LeftShin, B.LeftFoot, -1]]) {
    ...
    assert(sign * knee.x > .44 && sign * knee.x < .57); assert(sign * foot.x > .48 && sign * foot.x < .64);
  ...
  validateReins(player, intersections); // 牦牛检查增加胸毛／额毛／面侧毛，原6毫米鼻环接触门槛不变。
}
/** 并入check:mounts，完整追加牦牛矩阵，不替换四物种任何既有回归。 */
export function checkYak() {
  const d = mountDefinition('yak_black'), actor = d.createActor(), player = createMountPlayer(actor, d);
  ...
  assert.deepEqual(actor.stats, YAK_AUTHOR_STATS);
  assert.equal(validateMountMesh(actor), 30); assert.equal(YAK_JOINTS.length, 29);
  YAK_JOINTS.forEach((joint, i) => assert(joint.parent < i && (i === 0 ? joint.parent === -1 : joint.parent >= 0)));
  const anatomy = checkYakAnatomy(actor); faults += anatomy.faults;
  ...
  const feet = new Set(actor.data.triangles.filter(face => face.part.endsWith('Hoof')).flatMap(face => face.indices));
  const heads = new Set(actor.data.triangles.filter(face => face.part === 'Head' || face.part === 'NoseMirror').flatMap(face => face.indices));
  const furIds = new Set(actor.data.triangles.filter(f => ['Body', 'ChestFur', 'Forelock', 'LeftCheekFur', 'RightCheekFur', 'TailPlume'].includes(f.part)).flatMap(f => f.indices));
  ...
  for (const motion of MOUNT_MOTIONS) {
    console.log('Checking yak body', motion); player.select(motion); const clip = player.clips.get(motion)!;
    ...
    let foot = Infinity, head = Infinity, fur = Infinity, first: Vector3[] = [];
    const trajectories = ['FrontLeft', 'FrontRight', 'BackLeft', 'BackRight'].map(leg => ({ leg, bone: actor.bones[yakBone(`${leg}Foot`)], points: [] as Vector3[] }));
    for (let i = 0; i <= 240; i++) {
      ...
      for (const j of furIds) fur = Math.min(fur, points[j].y);
      checkYakFurAttachments(actor, true);
      near(actor.bones[1].position.x, 0); near(actor.bones[1].position.z, YAK_JOINTS[1].bindWorld[2]);
    ...
    if (motion === 'eat') assert(head >= .01 && head <= .12, `yak Eat head outside .01–.12m: ${head}`);
    assert(fur >= 0, `${motion}: fur through ground ${fur}`);
    ground[motion] = { foot, head, fur };
  ...
  for (const gender of BODY_TYPES) {
    const riding = createRidingPlayer(createRecipe({ bodyType: gender }), 'simple', 'yak_black');
    ...
    for (const saddle of ['simple', 'travel'] as const) {
      ...
      const seat = riding.seat.getWorldPosition(new Vector3()); assert(seat.z > -.25 && seat.z < -.02 && seat.y > 1.445 && seat.y < 1.51, 'seat must remain on the broad low yak pad');
      riding.select('pose'); checkRider(riding, true);
      for (const clip of RIDING_CLIP_IDS) {
        console.log('Checking yak rider', gender, saddle, clip); riding.select(clip); const first = riding.rider.bones.map(b => [...b.matrixWorld.elements]);
    ...
    for (const saddle of ['none', 'travel'] as const) {
      ...
      for (const id of ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'yak_black', 'horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'yak_black'] as const) {
    ...
    const mount = riding.mount; assert.throws(() => riding.setMount('missing_yak' as MountId)); assert(riding.mount === mount); faults++;
  ...
  assert.equal(wardrobeCases, BODY_TYPES.length * TOP_IDS.filter(id => id !== 'body').length * BOTTOM_IDS.filter(id => id !== 'body').length); assert.equal(swaps, 40);
  return { result: 'passed', mesh: { ...YAK_AUTHOR_STATS, shells: 30 }, anatomy, ground, gait, facial, bodyPoses, ridingPoses, wardrobeCases, swaps, faults,
    boundary: 'Authored in-place gait; sparse rein intersections include the broad body, nose mirror, horns and fur shells. Not full garment/saddle/contact or visual approval.' };
}
