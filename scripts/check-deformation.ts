...
for(const bodyType of BODY_TYPES){
  ...
  for(const bottom of BOTTOM_IDS){if(bottom==='body')continue;const p=makeTrousers(createRecipe({bodyType,slots:{bottom}}))!;assertKnees(p.mesh,true);assert.equal(triCount(p.mesh),220);}
...
const report={passed:true,bodyGeometryVersion:BODY_GEOMETRY_VERSION,rows,independentTrousersTriangles:220,mutationChecks:5,scope:'Authoring structure and injected regressions only. Actual FBX source-key/midpoint intersections and real screenshots remain separate checks; no automatic visual approval.'};
