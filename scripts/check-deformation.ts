...
for(const bodyType of BODY_TYPES){
  const character=makeCharacter(createRecipe({bodyType,slots:presetSlots('body')}));assert.equal(triCount(character.body),BODY_TRIANGLES);
  for(const bottom of BOTTOM_IDS){if(bottom==='body')continue;const p=makeTrousers(createRecipe({bodyType,slots:{bottom}}))!;assertKnees(p.mesh,true);assert.equal(triCount(p.mesh),bottom==='loose_trousers'?272:bottom==='guard_pants'?304:220);}
...
const report={passed:true,bodyGeometryVersion:BODY_GEOMETRY_VERSION,rows,independentTrousersTriangles:{loose_trousers:272,guard_pants:304,other:220},mutationChecks:5,scope:'Authoring structure and injected regressions only. Actual FBX source-key/midpoint intersections and real screenshots remain separate checks; no automatic visual approval.'};
