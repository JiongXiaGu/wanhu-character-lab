...
for(const look of WARDROBE_LOOKS)for(const bodyType of BODY_TYPES)for(const [height,build]of [[1.58,0],[1.76,.5],[1.92,1]]){
 ...
 for(const lod of [2] as WardrobeLod[]){const d=makeCharacter(recipe,{lod});assert.deepEqual(d.body,base.body);assert.deepEqual(d.joints,base.joints);assert.deepEqual(d.recipe,base.recipe);assertComponentWinding(d.surface);counts.push(triCount(d.surface));assert.equal(triCount(d.body),510);
 ...
 assert.equal(counts.length,1);for(const oldLod of [0,1,2] as WardrobeLod[])assert.deepEqual(makeCharacter(recipe,{lod:oldLod}),base,'旧LOD参数必须归一到标准模型');assert.deepEqual(makeCharacter(recipe),base);rows.push({look:look.id,bodyType,height,build,triangles:counts});
}
...
for(const id of priority){const source=JSON.parse(readFileSync(`public/mixamo/${id}.json`,'utf8'))as MixamoMotionData;
 for(const bodyType of BODY_TYPES)for(const look of ['plain-female','town-female','ceremony-female'])for(const lod of [2] as WardrobeLod[]){
...
const report={schema:'wanhu-tailoring-v2.1',geometryVersion:GARMENT_GEOMETRY_VERSION,testedSha:process.env.REVIEW_HEAD_SHA??'local',inventoryFiles:inventory.totalFiles,staticVariants:rows.length,rows,priority,frames,vertices,failures,passed:failures.length===0,visualApproval:false,note:'Topology, weights, complete time sampling and LOD are checked. These tests do not prove absence of self-intersection. Review actual images and complete timelines separately.'};
