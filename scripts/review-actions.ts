...
async function open(action:string,view='free',display='beauty',phase=.25) {
  ...
  await page.evaluate(p=>window.__WANHU_REVIEW__!.seek(p),phase);await page.waitForTimeout(60);
...
try {
  for(const id of WORK_IDS){
    ...
    for(const phase of [0,.2,.38,.52,.75,def.loop?.999:1]){
      await page.evaluate(p=>window.__WANHU_REVIEW__!.seek(p),phase);await page.waitForTimeout(25);
  ...
  await page.waitForTimeout(200);assert.equal(await page.evaluate(()=>window.__WANHU_REVIEW__!.getStatus().phase),1,'单次动作完成后循环了');
  ...
  await page.getByLabel('动画进度',{exact:true}).evaluate(el=>{Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value')!.set!.call(el,'0.52');el.dispatchEvent(new Event('input',{bubbles:true}));});await page.waitForTimeout(80);assert.equal(await page.evaluate(()=>window.__WANHU_REVIEW__!.getStatus().eventCount),0);
