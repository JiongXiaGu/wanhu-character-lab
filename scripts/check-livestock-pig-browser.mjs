...
export async function checkPigBrowser(page,base,dir,screenshots,setPhase) {
  const animal='pig_domestic_black',budgets=[['lod0',216,128],['lod1',138,89],['lod2',82,57]];
  ...
  assert.deepEqual(await page.getByLabel('家畜种类',{exact:true}).locator('option').evaluateAll(nodes=>nodes.map(n=>n.value)),['chicken_brown','duck_domestic_brown','goose_domestic_white',animal,'dog_rural_yellow', 'cat_rural_orange']);
  for(const [lod,triangles,logicalVertices] of budgets) {
    ...
    for(const motion of ['idle','walk','run','root','sniff','sleep']) {
  ...
  if(screenshots) {
    await page.getByTestId('livestock-view-three').click();
    await page.locator('.livestock-viewport canvas').hover();await page.mouse.wheel(0,-180);await page.waitForTimeout(250);
    await shot('pig-eyes.png');await page.getByTestId('livestock-view-left').click();
  }
  ...
  for(const id of ['chicken_brown','duck_domestic_brown','goose_domestic_white','dog_rural_yellow',animal]) {
  ...
  for(const count of [1,10,50,100,500]) {
    ...
    if(count===100||count===500){await page.getByTestId('livestock-view-farm').click();await shot(`pig-farm-${count}.png`);}
  }
