...
try {
  ...
  assert.deepEqual(await page.getByTestId('mount-horse').locator('option').evaluateAll(nodes => nodes.map(n => n.value)), ['horse_chestnut', 'donkey_gray']);
  ...
  checks.push('two real species, two equipment fields, body motion semantics and phase survive switching; all four donkey clips work without saddle');
