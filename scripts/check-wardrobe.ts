...
const report={schema:WARDROBE_VERSION,geometryVersion:GARMENT_GEOMETRY_VERSION,bodyHideVersion:BODY_HIDE_VERSION,sourceSha:process.env.REVIEW_HEAD_SHA??'local',staticVariants:rows.length,pairwiseCombinations:combinations,looks:WARDROBE_LOOKS.length,bodyTypes:BODY_TYPES.length,clips:MIXAMO_CLIPS.length,phasesPerClip:9,sampledFrames:frames,vertexSamples,wrapEdgesChecked,coveredLegCases,invalidFilesRejected:bads.length,rows,passed:true,note:'Finite values, winding, weights, continuous garment topology and covered-body restoration are checked. This is not a collision/cloth/Unity performance test.'};
