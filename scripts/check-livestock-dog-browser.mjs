...
export async function checkDogBrowser(page,base,dir,screenshots,setPhase) {
  ...
  for(const [lod,triangles,logicalVertices] of budgets) {
    ...
    if(lod==='lod0') {
      ...
      if(screenshots) {
        await page.getByTestId('livestock-view-front').click();await shot('dog-front.png');
        await page.getByTestId('livestock-view-farm').click();await shot('dog-farm-single.png');
    ...
    for(const motion of ['idle','walk','run','sniff','bark','sleep']) {
      ...
      if(lod==='lod0'&&['walk','run','sniff','bark'].includes(motion)) {await page.getByTestId('livestock-view-left').click();await shot(`dog-${motion}.png`);}
    }
  }
  // 通过已存在的相机临时状态恢复严格正交侧视，不改正式相机预设或动物光照。
  const stored=await page.evaluate(()=>sessionStorage.getItem('wanhu.livestock.preview.v1'));
  await page.evaluate(id=>sessionStorage.setItem('wanhu.livestock.preview.v1',JSON.stringify({version:1,animal:id,count:1,surface:'land',motion:'idle',playing:false,loop:true,lod:'lod0',phase:.5,seed:731,view:'left',camera:{position:[-3,.40,0],target:[0,.40,0],zoom:1}})),animal);
  ...
  await shot('dog-side.png');

  await page.getByTestId('livestock-motion-bark').click();await pause();await setPhase(.5);await shot('dog-bark.png');
  await page.getByTestId('livestock-view-three').click();await shot('dog-bark-three.png');
  ...
  await page.goto(`${base}/?lab=livestock&animal=missing_dog&clip=bark&paused=1`,{waitUntil:'networkidle'});await page.waitForFunction(()=>window.__LIVESTOCK_REVIEW__?.snapshot().animal==='chicken_brown');assert.equal((await snap()).motion,'idle');
