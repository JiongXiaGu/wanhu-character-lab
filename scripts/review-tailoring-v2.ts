...
const context=await browser.newContext({viewport:{width:1600,height:1000},deviceScaleFactor:1});const page=await context.newPage(),errors:string[]=[],shots:any[]=[],videos:string[]=[];
...
}catch(error){failure=String(error);throw error;}finally{await context.close();await browser.close();await writeFile(dir+'/visual.json',JSON.stringify({testedSha:process.env.REVIEW_HEAD_SHA,priority,shots,videos,errors,failure,passed:!failure&&!errors.length,generated:true,manuallyReviewed:false},null,2));}
