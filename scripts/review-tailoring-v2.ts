...
async function open(params:Record<string,string>){await page.goto(base+'/?'+new URLSearchParams({review:'1',paused:'1',headwear:'none',leftHand:'none',rightHand:'none',back:'none',...params}));await page.waitForFunction(()=>!!window.__WANHU_REVIEW__?.getStatus().motion?.ready);assert.equal(await page.locator('[role=alert]').count(),0);}
...
try{
 ...
 for(const clip of MIXAMO_CLIPS){await open({bodyType:'female',look:'town-female',motion:clip.id,view:'front'});await shot('library-'+clip.id,.5,{clip:clip.id,kind:'library'});}
 for(const bodyType of ['male','female'])for(const look of ['plain-female','town-female','ceremony-female']){
  for(const view of ['front','side','back']){await open({bodyType,look,motion:'pilot-switches',view});for(const phase of [0,.25,.5,.75,1])await shot(`${bodyType}-${look}-standard-pilot-${view}-${phase}`,phase,{bodyType,look,view,clip:'pilot-switches'});}
 }
 for(const bodyType of ['male','female'])for(const id of priority.filter(id=>id!=='pilot-switches')){
  await open({bodyType,look:'ceremony-female',motion:id,view:'side'});for(let k=0;k<=16;k++)await shot(`${bodyType}-${id}-sequence-${k}`,k/16,{bodyType,clip:id,kind:'sequence'});
 }
 for(const bodyType of ['male','female']){await open({bodyType,look:'town-female',motion:'pilot-switches',view:'front',display:'clay'});await shot(`clay-${bodyType}-standard`, .5,{bodyType,kind:'clay'});}
 // 交互检查：搜索、收藏、筛选空结果、快捷坐姿及固定基模换装时序保持。
 await open({bodyType:'female',look:'town-female',motion:'pilot-switches',view:'front'});
 await page.getByLabel('搜索动画',{exact:true}).fill('劳动');assert.equal(await page.getByTestId('motion-snatch').count(),1);
 ...
 for(const bodyType of ['male','female'])for(const id of ['pilot-switches','shooting-arrow','jogging','snatch']){
  ...
  try{p.on('pageerror',e=>errors.push(e.message));await p.goto(base+'/?'+new URLSearchParams({review:'1',paused:'1',bodyType,motion:id,look:'ceremony-female',view:'side',headwear:'none'}));await p.waitForFunction(()=>!!window.__WANHU_REVIEW__?.getStatus().motion?.ready);assert(await p.getByRole('checkbox',{name:'人物动画循环播放',exact:true}).isChecked(),'人物动画默认循环未启用');if(id!=='jogging')await p.getByRole('checkbox',{name:'人物动画循环播放',exact:true}).uncheck();await p.getByRole('button',{name:'播放',exact:true}).click();
