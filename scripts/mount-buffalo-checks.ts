...
import { MOUNT_IDS, MOUNT_MOTIONS, type MountActor, type MountId } from '../src/mounts/types';
import { BUFFALO_JOINTS, buffaloBone } from '../src/buffalo/rig';
...
import { BUFFALO_AUTHOR_STATS, checkBuffaloAnatomy } from './mount-buffalo-anatomy-checks';
...
function facialContact(actor: MountActor, part: string): number {
  ...
  for (const i of ids) {
    const vertex = actor.data.vertices[i]; assert.deepEqual(vertex.weight, [buffaloBone('Head'), buffaloBone('Head'), 1]); center.add(new Vector3(...vertex.position));
  }
...
function checkRider(player: RidingPlayer, intersections: boolean) {
  ...
  assert.equal(player.mount.bones.length, 28); assert.equal(player.rider.bones.length, 20);
  ...
  assert(player.seat.parent === player.mount.bones[buffaloBone('Spine')]);
  ...
  for (const [k, f, sign] of [[B.RightShin, B.RightFoot, 1], [B.LeftShin, B.LeftFoot, -1]]) {
    ...
    assert(sign * knee.x > .40 && sign * knee.x < .56); assert(sign * foot.x > .44 && sign * foot.x < .63);
  ...
  validateReins(player, intersections); // 包含宽头鼻镜、短颈、桶身、角和垂皮；不放宽原6毫米嘴环接触门槛。
}
/** 并入check:mounts，旧灰驴矩阵保留，不能以新物种检查替换老回归。 */
export function checkBuffalo() {
  const d = mountDefinition('buffalo_water'), actor = d.createActor(), player = createMountPlayer(actor, d);
  ...
  assert.deepEqual(actor.stats, BUFFALO_AUTHOR_STATS);
  assert.equal(validateMountMesh(actor), 27); assert.equal(BUFFALO_JOINTS.length, 28);
  BUFFALO_JOINTS.forEach((joint, i) => assert(joint.parent < i && (i === 0 ? joint.parent === -1 : joint.parent >= 0)));
  const anatomy = checkBuffaloAnatomy(actor); faults += anatomy.faults;
  ...
  for (const motion of MOUNT_MOTIONS) {
    console.log('Checking buffalo body', motion); player.select(motion); const clip = player.clips.get(motion)!;
    ...
    assert.equal(clip.tracks.length, 28); assert(!clip.tracks.some(track => track.name.startsWith('Root.') || track.name.endsWith('.scale')));
    ...
    const trajectories = ['FrontLeft', 'FrontRight', 'BackLeft', 'BackRight'].map(leg => ({ leg, bone: actor.bones[buffaloBone(`${leg}Foot`)], points: [] as Vector3[] }));
    for (let i = 0; i <= 240; i++) {
      ...
      near(actor.bones[1].position.x, 0); near(actor.bones[1].position.z, BUFFALO_JOINTS[1].bindWorld[2]);
    ...
    if (motion === 'eat') assert(head >= .01 && head <= .12, `buffalo Eat head outside .01–.12m: ${head}`);
  ...
  for (const gender of BODY_TYPES) {
    const riding = createRidingPlayer(createRecipe({ bodyType: gender }), 'simple', 'buffalo_water');
    ...
    for (const saddle of ['simple', 'travel'] as const) {
      ...
      const seat = riding.seat.getWorldPosition(new Vector3()); assert(seat.z > -.30 && seat.z < .30 && seat.y > 1.36 && seat.y < 1.43, 'seat must remain on the broad low buffalo pad');
      riding.select('pose'); checkRider(riding, true);
      for (const clip of RIDING_CLIP_IDS) {
        console.log('Checking buffalo rider', gender, saddle, clip); riding.select(clip); const first = riding.rider.bones.map(b => [...b.matrixWorld.elements]);
    ...
    for (const saddle of ['none', 'travel'] as const) {
      ...
      for (const id of [...MOUNT_IDS.filter(id => id !== 'buffalo_water'), 'buffalo_water', ...MOUNT_IDS.filter(id => id !== 'buffalo_water'), 'buffalo_water'] as MountId[]) {
    ...
    const mount = riding.mount; assert.throws(() => riding.setMount('missing_buffalo' as MountId)); assert(riding.mount === mount); faults++;
  ...
  assert.equal(wardrobeCases, BODY_TYPES.length * TOP_IDS.filter(id => id !== 'body').length * BOTTOM_IDS.filter(id => id !== 'body').length); assert.equal(swaps, 8 * MOUNT_IDS.length);
  return { result: 'passed', mesh: { ...BUFFALO_AUTHOR_STATS, shells: 27 }, anatomy, ground, gait, facial, bodyPoses, ridingPoses, wardrobeCases, swaps, faults,
    boundary: 'Authored in-place gait; sparse rein intersections include the broad body, nose mirror, horns and dewlap. Not full garment/saddle/contact or visual approval.' };
}
