...
try{
 ...
 for(const look of WARDROBE_LOOKS){
  ...
  for(const bodyType of ['male','female'])for(const [height,build]of[['1.58','0'],['1.92','1']]){await open({look:look.id,bodyType,height,build,view:'three'});await shot(`${look.id}-${bodyType}-${height}`);}
 ...
 let after=await recipe();assert.equal(after.slots.headwear,before.slots.headwear);assert.deepEqual(after.dyes,before.dyes);assert.equal(after.height,before.height);await shot('random-locked');
 ...
 assert.equal(records.length,159);assert.equal(videos.length,4);assert.deepEqual(errors,[]);
