...
export async function checkBuffaloBrowser(page, base, checks) {
  ...
  await page.goto(`${base}/?lab=mount&mount=buffalo_water&pose=bind&paused=1`);
  ...
  await page.goto(`${base}/?lab=mount&mount=buffalo_water&clip=Buffalo_Walk&paused=1&phase=.375`);
  await page.waitForFunction(() => window.__MOUNT_REVIEW__?.mountId() === 'buffalo_water');
  const first = await body(); assert.equal(first.stats.bones, 28); assert.equal(first.stats.triangles, 2056); near(first.status.duration, 2.0); near(first.status.phase, .375);
  assert.equal(await page.getByTestId('mount-stage-name').innerText(), '水牛');
  ...
  for (const [clip, duration] of [['Buffalo_Idle', 5.8], ['Buffalo_Walk', 2.0], ['Buffalo_Run', 1.15], ['Buffalo_Eat', 7.2]]) {
  ...
  await page.getByTestId('animal-mode-riding').click(); await page.waitForFunction(() => window.__RIDING_REVIEW__?.mountId() === 'buffalo_water');
  ...
  checks.push('buffalo native URL and four independent clips; 2056 triangles/28 bones; two fields/three animal sections/one canvas; no-saddle body-to-riding does not auto-equip');
  ...
  for (const [id, bones, duration] of [['horse_chestnut', 25, 1.2], ['donkey_gray', 27, 1.4], ['camel_bactrian', 29, 1.6], ['cattle_yellow', 28, 1.8], ['yak_black', 29, 1.95], ['buffalo_water', 28, 2.0]]) {
  ...
  for (const id of ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'yak_black', 'buffalo_water']) {
  ...
  for (const gender of ['female', 'male']) {
    ...
    for (const saddle of ['simple', 'travel']) {
      ...
      for (const [clip, duration] of [['Rider_Idle', 5.8], ['Rider_Walk', 2.0], ['Rider_Run', 1.15]]) {
  ...
  checks.push('buffalo male/female × simple/travel × three rider clips; species preserve rider/appearance/camera/phase; none freezes across all six species; wardrobe and replay remain valid');
  await page.getByTestId('animal-mode-horse').click(); await page.waitForFunction(() => window.__MOUNT_REVIEW__?.mountId() === 'buffalo_water'); assert.equal(await page.getByTestId('mount-saddle').inputValue(), 'travel');
  await page.getByTestId('animal-mode-riding').click(); await page.waitForFunction(() => window.__RIDING_REVIEW__?.mountId() === 'buffalo_water'); assert.equal(await page.getByTestId('mount-saddle').inputValue(), 'travel');
  assert.equal(await page.locator('canvas').count(), 1); checks.push('buffalo body/riding round trip preserves water buffalo species and whole saddle kit');
  ...
  await page.getByTestId('animal-mode-horse').click(); await page.waitForFunction(() => window.__MOUNT_REVIEW__?.mountId() === 'buffalo_water');
  await page.getByTestId('Buffalo_Eat').click(); await pause('horse-play'); await page.evaluate(() => window.__MOUNT_REVIEW__.seek(.625)); await page.getByTestId('horse-view-front').click();
  ...
  checks.push('buffalo real page round trips restore each mode phase, clip, paused state, Orbit camera/zoom and unsaved V5 rider appearance without writing wardrobe storage');
  await page.evaluate(() => sessionStorage.setItem('wanhu.mount.preview.v1.riding', '{bad-cache'));
  await page.goto(`${base}/?lab=riding&mount=buffalo_water&saddle=simple&clip=Rider_Walk&paused=1&phase=.2&preview=resume`); await page.waitForFunction(() => window.__RIDING_REVIEW__?.mountId() === 'buffalo_water');
