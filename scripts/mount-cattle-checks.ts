...
import { CATTLE_JOINTS, cattleBone } from '../src/cattle/rig';
...
function facialContact(actor: MountActor, part: string): number {
  ...
  for (const i of ids) {
    const vertex = actor.data.vertices[i]; assert.deepEqual(vertex.weight, [cattleBone('Head'), cattleBone('Head'), 1]); center.add(new Vector3(...vertex.position));
  }
...
function checkRider(player: RidingPlayer, intersections: boolean) {
  ...
  assert(player.seat.parent === player.mount.bones[cattleBone('Spine')]);
...
export function checkCattle() {
  const d = mountDefinition('cattle_yellow'), actor = d.createActor(), player = createMountPlayer(actor, d);
  ...
  assert.deepEqual(actor.stats, CATTLE_AUTHOR_STATS);
  assert.equal(validateMountMesh(actor), 27); assert.equal(CATTLE_JOINTS.length, 28);
  CATTLE_JOINTS.forEach((joint, i) => assert(joint.parent < i && (i === 0 ? joint.parent === -1 : joint.parent >= 0)));
  const anatomy = checkCattleAnatomy(actor); faults += anatomy.faults;
  ...
  for (const motion of MOUNT_MOTIONS) {
    console.log('Checking cattle body', motion); player.select(motion); const clip = player.clips.get(motion)!;
    ...
    const trajectories = ['FrontLeft', 'FrontRight', 'BackLeft', 'BackRight'].map(leg => ({ leg, bone: actor.bones[cattleBone(`${leg}Foot`)], points: [] as Vector3[] }));
    for (let i = 0; i <= 240; i++) {
      ...
      near(actor.bones[1].position.x, 0); near(actor.bones[1].position.z, CATTLE_JOINTS[1].bindWorld[2]);
    ...
    if (motion === 'eat') assert(head >= .01 && head <= .12, `cattle Eat head outside .01–.12m: ${head}`);
  ...
  for (const gender of BODY_TYPES) {
    const riding = createRidingPlayer(createRecipe({ bodyType: gender }), 'simple', 'cattle_yellow');
    ...
    for (const saddle of ['simple', 'travel'] as const) {
      ...
      const seat = riding.seat.getWorldPosition(new Vector3()); assert(seat.z > -.30 && seat.z < .30 && seat.y > 1.46 && seat.y < 1.53, 'seat must remain on the broad low cattle pad');
      riding.select('pose'); checkRider(riding, true);
      for (const clip of RIDING_CLIP_IDS) {
        console.log('Checking cattle rider', gender, saddle, clip); riding.select(clip); const first = riding.rider.bones.map(b => [...b.matrixWorld.elements]);
    ...
    for (const saddle of ['none', 'travel'] as const) {
      ...
      for (const id of ['horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow', 'horse_chestnut', 'donkey_gray', 'camel_bactrian', 'cattle_yellow'] as const) {
    ...
    const mount = riding.mount; assert.throws(() => riding.setMount('missing_cattle' as MountId)); assert(riding.mount === mount); faults++;
  ...
  assert.equal(wardrobeCases, BODY_TYPES.length * TOP_IDS.filter(id => id !== 'body').length * BOTTOM_IDS.filter(id => id !== 'body').length); assert.equal(swaps, 32);
  return { result: 'passed', mesh: { ...CATTLE_AUTHOR_STATS, shells: 27 }, anatomy, ground, gait, facial, bodyPoses, ridingPoses, wardrobeCases, swaps, faults,
