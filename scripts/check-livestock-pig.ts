...
function visualLandmarks(data:AnimalMeshData) {
  ...
  for(const part of data.parts.filter(p=>p.name.startsWith('Eye'))) {
    ...
    assert.equal(part.count,6,'猪眼使用贴合头面的小型闭合眼体，不恢复装饰性多圈壳');
    assert(span(points,1)<.05&&Math.hypot(span(points,0),span(points,2))<.08,'眼睛不得扩大成黑眼圈');
    const colors=data.colors.slice(part.start,part.start+part.count);
    assert(colors.filter(c=>luminance(c)>=.015).length<=1,'只允许上眼缘一个暖灰点，不能恢复亮色环');
    assert(colors.every(c=>luminance(c)<.16),'眼缘不能成为高亮饰片');
    let darkArea=0;
    for(let i=0;i<data.indices.length;i+=3) {
    ...
    assert(darkArea>.00075,'深色眼体不能缩成针尖');
  }
}
...
assert.deepEqual(definition.motions.map(m=>m.id),['idle','walk','run','root','sniff','sleep']);
...
for(const lod of definition.lods) {
  ...
  if(lod.id!=='lod2') {
    ...
    fails(()=>visualLandmarks(broken));
    broken=clone();for(let i=eye.start+1;i<eye.start+5;i++)broken.colors[i]='#898271';
    fails(()=>visualLandmarks(broken));
    broken=clone();for(let i=eye.start;i<eye.start+eye.count;i++)broken.positions[i]=new Vector3(...broken.positions[i]).sub(center).multiplyScalar(2).add(center).toArray();
    fails(()=>visualLandmarks(broken));
