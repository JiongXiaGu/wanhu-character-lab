...
function attachmentWitnesses(data:AnimalMeshData) {
  return data.parts.slice(1).map(part=>{
    ...
    if(bone>=B.FrontLegL)return {name:part.name,indices:indices.filter(i=>data.positions[i][1]>.39),average:true};
    ...
    if(part.name.startsWith('Nostril'))return {name:part.name,indices:indices.slice(0,3),average:true};
...
const report={result:'passed',sourceSHA:process.env.REVIEW_HEAD_SHA??'local',animal:definition.id,bones:9,weights:1,totalPoses:3*5*241,totalFaultInjections:36,reports};
