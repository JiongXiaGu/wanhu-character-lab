...
import { DOG_DEFINITION as definition } from '../src/dog/definition';
import { DOG_BONES as B, DOG_JOINTS, DOG_LEGS } from '../src/dog/rig';
...
function weights(data:AnimalMeshData) {
  ...
  for(const part of data.parts.slice(1)) {
    const expected=part.name==='Tail'?B.Tail:DOG_JOINTS.findIndex(j=>j.name===part.name);
  ...
  assert(data.bones.slice(0,data.parts[0].count).includes(B.Neck),'犬颈不能改为Body刚性壳');
  assert(data.bones.slice(0,data.parts[0].count).includes(B.Head),'犬头鼻不能失去Head权重');
  for(let i=0;i<data.parts[0].count;i++) {
    const z=data.positions[i][2];assert.equal(data.bones[i],z<.20?B.Body:z<.30?B.Neck:B.Head,'主体截面权重');
  }
}
function validate(data:AnimalMeshData,lod:LivestockLodDefinition) {
  ...
  const main=data.positions.slice(0,data.parts[0].count),nose=main.filter(p=>p[2]>.58);
  assert.equal(data.parts[0].name,'BodyNeckHeadMuzzle');
  assert(Math.max(...main.map(p=>p[2]))<.62&&nose.length>=3,'中长嘴筒不能脱离或过长');
  assert(Math.max(...main.map(p=>p[0]))-Math.min(...main.map(p=>p[0]))>.30,'胸腹过细');
  assert(Math.max(...main.map(p=>p[0]))-Math.min(...main.map(p=>p[0]))<.36,'不能把犬做成猪的桶身');
  const body=main.filter((_,i)=>data.bones[i]===B.Body);
  assert(Math.min(...body.map(p=>p[1]))>.27&&Math.max(...body.map(p=>p[1]))<.71,'正常长腿与胸腹留空');
  for(const point of data.positions)assert(data.positions.some(p=>Math.hypot(p[0]+point[0],p[1]-point[1],p[2]-point[2])<.001),'左右结构异常');
  for(const bone of DOG_LEGS){const [x,y,z]=DOG_JOINTS[bone].position;assert(Math.abs(x)>.10&&Math.abs(x)<.14&&y>.48);assert(bone<=B.FrontLegR?z>.15:z<-.25,'肩臀腿位');}
}
function visualLandmarks(data:AnimalMeshData) {
  ...
  const main=data.positions.slice(0,data.parts[0].count),head=main.filter((_,i)=>data.bones[i]===B.Head);
  assert(span(head,2)>.15&&span(head,2)<.28,'楔形头与中等嘴筒');
  assert(span(head,0)>.21&&span(head,0)<.26,'头部宽度不能狼化或过度卡通');
  for(const bone of DOG_LEGS) {
    const part=data.parts.find(p=>p.name===DOG_JOINTS[bone].name)!,points=data.positions.slice(part.start,part.start+part.count);
    assert(span(points,1)>.51&&span(points,0)>.065,'四条正常长腿必须完整保留');
    assert(points.filter(p=>p[1]<.01).length>=3,'脚底不能降成针尖');
  }
  const tail=data.parts.find(p=>p.name==='Tail')!,tp=data.positions.slice(tail.start,tail.start+tail.count);
  assert(Math.max(...tp.map(p=>p[1]))>.76&&span(tp,2)>.08,'上扬弯尾剪影');
  for(const ear of data.parts.filter(p=>p.name.startsWith('Ear'))) {
    const points=data.positions.slice(ear.start,ear.start+ear.count);
    assert(Math.max(...points.map(p=>p[1]))>.80&&Math.max(...points.map(p=>p[1]))<.82,'朴素立耳不能消失或夸张放大');
  }
  for(const eye of data.parts.filter(p=>p.name.startsWith('Eye'))) {
    const points=data.positions.slice(eye.start,eye.start+eye.count);
    assert(span(points,1)>.017&&span(points,1)<.04,'低模犬眼的可读尺度');
    assert(data.colors.slice(eye.start,eye.start+eye.count).every(c=>new Color(c).getHSL({h:0,s:0,l:0}).l<.10),'不能加白眼圈');
  }
}
...
function attachmentWitnesses(data:AnimalMeshData) {
  return data.parts.slice(1).map(part=>{
    ...
    if(bone>=B.FrontLegL)return {name:part.name,indices:indices.filter(i=>data.positions[i][1]>.50),average:true};
...
function attachments(data:AnimalMeshData,points:Vector3[]) {
  ...
  for(const witness of attachmentWitnesses(data)) {
    ...
    if(DOG_JOINTS.some(j=>j.name===witness.name))for(const id of witness.indices)
...
function inverseBind(actor:ReturnType<typeof createAnimalActor>) {
  actor.skeleton.boneInverses.forEach((matrix,i)=>{const expected=new Matrix4().makeTranslation(...DOG_JOINTS[i].position).invert();matrix.elements.forEach((n,j)=>assert(Math.abs(n-expected.elements[j])<1e-6,'作者inverse bind不符'));});
...
for(const lod of definition.lods) {
  ...
  const soles=DOG_LEGS.map(b=>data.indices.map((id,i)=>data.bones[id]===b&&data.positions[id][1]<.01?i:-1).filter(i=>i>=0));
  const nose=data.indices.map((id,i)=>id<data.parts[0].count&&data.positions[id][2]>.58?i:-1).filter(i=>i>=0);
  ...
  let poses=0,minGround=Infinity,minSniffNose=Infinity,minBodyAreaRatio=Infinity,maxHorizontalRadius=0,maxBarkNose=-Infinity;
  ...
  for(const motion of definition.motions) {
    ...
    for(let f=0;f<=240;f++) {
      ...
      for(const foot of soles){const min=Math.min(...foot.map(i=>points[i].y));assert(min>=-.001&&min<.07,'四足接地／步幅抬脚范围');}
      if(motion.id==='bark'||motion.id==='sniff')for(const foot of soles)foot.forEach(i=>assert(points[i].distanceTo(bind[i])<1e-6,'吠叫／闻地四足必须稳定'));
      if(motion.id==='bark')maxBarkNose=Math.max(maxBarkNose,...nose.map(i=>points[i].y));
  ...
  assert(minSniffNose>=.001&&minSniffNose<=.12,`${lod.id}闻地鼻端高度：${minSniffNose}`);
  assert(maxBarkNose>.65,'警戒时头颈必须抬起，不能塌成闻地姿态');
  function forwardGait(sampler=sample) {
    ...
    for(let leg=0;leg<4;leg++) {
      ...
      for(const points of [a,b])assert(Math.abs(Math.min(...soles[leg].map(i=>points[i].y))-.007)<.001,'支撑脚滑离地面');
      const lifted=sampler(motion,phase(stance+(1-stance)/2));assert(Math.min(...soles[leg].map(i=>lifted[i].y))>.025,'摆动腿没有抬起');
    }
  }
  }
  forwardGait();
  ...
  const cached=crowd.cachedPoses;for(let i=0;i<40;i++)crowd.update(i/7,{motion:'sniff',surface:'land',mixed:true,loop:true},lod.id);
  ...
  broken=clone();broken.positions=broken.positions.map(p=>p[2]>.58?[p[0],p[1],p[2]+.4] as Point:p);fails(()=>validate(broken,lod));
  ...
  broken=clone();for(let i=0;i<broken.parts[0].count;i++)if(broken.bones[i]===B.Head)broken.bones[i]=B.Body;
  fails(()=>weights(broken));
  broken=clone();broken.positions[0]=[NaN,0,0];fails(()=>topology(broken));
  broken=clone();const leg=broken.parts.find(p=>p.name==='FrontLegL')!;
  for(let i=leg.start;i<leg.start+leg.count;i++){const[x,y,z]=broken.positions[i];broken.positions[i]=[x,y*.35,z];}
  fails(()=>visualLandmarks(broken));
  broken=clone();const tail=broken.parts.find(p=>p.name==='Tail')!;
  for(let i=tail.start;i<tail.start+tail.count;i++){const[x,y,z]=broken.positions[i];broken.positions[i]=[x,.6+(y-.6)*.2,z];}
  fails(()=>visualLandmarks(broken));
  fails(()=>forwardGait((motion,phase)=>sample(motion,1-phase)));
  if(lod.id!=='lod2') {
    broken=clone();const eye=broken.parts.find(p=>p.name==='EyeL')!;
    for(let i=eye.start;i<eye.start+eye.count;i++)broken.colors[i]='#ffffff';fails(()=>visualLandmarks(broken));
    broken=clone();for(let i=eye.start;i<eye.start+eye.count;i++){const[x,y,z]=broken.positions[i];broken.positions[i]=[x+1,y,z];}
    fails(()=>attachments(broken,broken.positions.map(p=>new Vector3(...p))));
  }
