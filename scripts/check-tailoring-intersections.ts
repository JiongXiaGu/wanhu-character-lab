...
const ids=['pilot-switches','shooting-arrow','jogging',...MIXAMO_CLIPS.filter(c=>c.id==='snatch').map(c=>c.id)],rows:any[]=[],failures:any[]=[];
let checkedFrames=0,pairsChecked=0;
for(const bodyType of ['male','female'] as const)for(const look of ['plain-female','town-female','ceremony-female'])for(const lod of [0,1,2] as WardrobeLod[]){
 const d=makeCharacter(applyLook(cleanRecipe({bodyType}),look),{lod}),c=d.surface,actor=makeActor(d),indices:number[][]=[];
