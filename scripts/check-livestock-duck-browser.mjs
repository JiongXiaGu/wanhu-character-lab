...
export async function checkDuckBrowser(page, base, dir, screenshots, setPhase) {
  ...
  assert.deepEqual(await page.getByLabel('家畜种类',{exact:true}).locator('option').evaluateAll(nodes=>nodes.map(n=>n.value)),['chicken_brown',animal,'goose_domestic_white','pig_domestic_black','dog_rural_yellow', 'cat_rural_orange']);
  for(const [lod,triangles,logicalVertices] of budgets) {
    for(const [surface,motions] of [['land',['idle_land','walk','run','feed_land','sleep']],['water',['idle_water','swim','dabble']]]) {
