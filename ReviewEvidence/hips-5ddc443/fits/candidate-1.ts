...
export function rebuildSeat(c:Cage):void {
  ...
  for(const side of ['Right','Left'] as const){
    ...
    const seat=root.map((a,i)=>{
      ...
      const w:Weight=i===5?[thigh,opposite,0.65]:[B.Hips,thigh,0.15];
  ...
  const along=(to:number)=>add(mul(center,0.6799999999999999),mul(c.vertices[to].p,0.32));
  ...
  const r=vertex(c,'SeatGusset.Right',pr,[B.RightThigh,B.LeftThigh,0.548]);
  const l=vertex(c,'SeatGusset.Left',pl,[B.LeftThigh,B.RightThigh,0.548]);
