...
export function rebuildSeat(c:Cage):void {
  ...
  for(const side of ['Right','Left'] as const){
    ...
    const seat=root.map((a,i)=>{
      ...
      const w:Weight=i===5?[thigh,opposite,0.85]:[B.Hips,thigh,0.35];
  ...
  const r=vertex(c,'SeatGusset.Right',pr,[B.RightThigh,B.LeftThigh,0.612]);
  const l=vertex(c,'SeatGusset.Left',pl,[B.LeftThigh,B.RightThigh,0.612]);
