...
当前形变任务：保留裸模膝部原值，裤装删除膝后压薄和冗余过渡环，252→220tris；人体保持510tris，中央裆点采用双大腿权重候选。服装几何 `wanhu-modular-garments-v3`，皮肤几何/蒙皮 `wanhu-skin-cage-v2`，绑定仍为 `wanhu-fixed-bodies-v1`。这不是已经确认的最终美术，实际状态见[本轮任务](Documentation/膝部与裆底修正.md)及[交接](Documentation/工作交接.md)。
...
它是截图命令，不会自动替人审图，也不能代替全部数值和交互测试。若当前环境无法运行完整浏览器，只允许明确标注的静态源码几何重建用于静态外观判断，不能冒充WebGL或FBX验证。本对话的容器可用性和runner检查状态分别记录，详见[验收规范](Documentation/GitHubActions截图验收规范.md)。

## 自动检查与边界

执行 `check:retired`、`check:mesh`（含 `check:deformation`）、`build`、`check:mixamo`、`check:wardrobe`、`check:tailoring`。保留原六条正式Actions，合并网格/蒙皮/换装改动与阶段收尾时统一核对。Modular Garment Review保留原服饰对照，再增加本地命令烟雾测试和156组裸模/裤装前后对照。没有第七条永久工作流，不部署Vercel/Visual，不录制视频；原结束/循环和源键/中点检查保留。

原身体/绑定黄金文件不刷新。测试明确列出唯一的裆点权重变化，并验证所有其他人体数据与完整绑定哈希不变；这不是新视觉基线。数值通过不意味着全部姿态零穿插。当前尚无外部服装Mesh导入器、Unity插件或GPU群体运行实现。
